    ...
    <div class="container-fluid mt-4">
        <div class="card">
            <div class="card-header d-flex justify-content-between bg-primary py-2 align-items-center">
                <h2 class="card-title m-0 text-white"> {{ __('Social_Links') }}</h2>
                <button class="btn btn-white" data-toggle="modal" data-target="#addsocialModal">
                    {{ __('Add').' '. __('Social_Links') }}
                </button>
            </div>
                                ...
                                <div class="modal-dialog modal-dialog-centered" role="document">
                                    <div class="modal-content">
                                        <div class="modal-header bg-primary py-2">
                                            <h4 class="modal-title text-white">{{ __('Edit') }} {{ __('Social_Links') }}</h4>
        ...
        <div class="modal fade" id="addsocialModal">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header bg-primary py-2">
                        <h4 class="modal-title text-white">{{ __('Add_New') }} {{ __('Social_Links') }}</h4>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <form action="{{ route('socialLink.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="form-group">
                                <label class="mb-0">{{ __('Thumbnail') }}</label>
                                <input type="file" name="photo" class="form-control-file" required>
                            </div>
                            <div class="form-group mb-0">
                                <label class="mb-0">{{ __('URL') }}</label>
                                <textarea name="url" class="form-control" placeholder="Enter Social Link URL" rows="2" required></textarea>
                            </div>
                        </div>
                        <div class="modal-footer py-2">
                            <button type="button" class="btn btn-secondary" data-dismiss="modal">{{ __('Close') }}</button>
